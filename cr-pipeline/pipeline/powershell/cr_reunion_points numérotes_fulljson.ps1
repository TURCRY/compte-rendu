...
function Invoke-LLM-OpenAICompat {
  ...
  if($ApiKey){
    ...
    $mask = $ApiKey.Substring(0,4) + "****" + $ApiKey.Substring($ApiKey.Length-4)
    Write-Host "[LLM] POST $uri" -ForegroundColor Cyan
    Write-Host ("[LLM] Authorization: Bearer {0} (len={1})" -f $mask, $ApiKey.Length) -ForegroundColor Cyan
  } else {
    Write-Host "[LLM] POST $uri SANS Authorization (ApiKey vide)" -ForegroundColor Yellow
  }

  $body = @{
    model     = $model      # ⬅️ ici on utilise bien le paramètre
    temperature = 0.2
    messages  = @(
...
- CONSERVEZ explicitement toutes les demandes de documents ou d'informations
  (pièces, plans, attestations, justificatifs, rapports, etc.) en les formulant comme des actions,
  par exemple : "fournir le plan de ...", "transmettre les relevés de ...", avec le responsable
  indiqué (expert, partie, avocat, etc.) et l'échéance si elle est mentionnée, sinon null.
'@
$Pass1_User_Template=@'
...
$Pass3_System=@'
Tu es un assistant de rédaction de compte rendu. À partir du JSON global fourni, produis un JSON FINAL NORMALISÉ avec ce schéma EXACT et STRICT :
{
  "date": "YYYY-MM-DD | null",
  "link": "string | null",
  "resume": "string (5–6 phrases, ton neutre)",
  "ordre_du_jour": ["string", "..."],
  "themes_abordes": [
    {
      "titre": "string",
      "synthese": ["string","..."],      // 5–10 points max, factuels
      "indices_source": [                // 1–3 justifications
        { "timecode": "HH:MM:SS", "speaker": "string", "extrait": "string court" }
      ]
    }
  ],
  ...
  "perspectives": [
    { "probleme": "string (1–2 phrases)", "solution": "string (1–2 phrases)" }
  ],
  "annexes": ["string","..."]
}
CONTRAINTES : aucun champ supplémentaire ; aucune invention ; renseigner null si info absente ; ≤4000 mots.
'@
$Pass3_User_Template=@'
JSON global à convertir vers le JSON FINAL normalisé :
{GLOBAL_JSON}

Métadonnées suggérées (si pertinentes) :
- Date : {DATE_SUGGEST}
- Link : {LINK_SUGGEST}

Renvoie uniquement le JSON FINAL conforme au schéma.
'@
...
(pièces, plans, diagnostics, attestations, justificatifs, photos, rapports, relevés, etc.)
...
- si aucune demande n’existe → "demandes_documents": [].
